...
def read_image(image_path):
    ...
    Y = os.path.join(image_path,"..", "..", 'Y')
    label_path = os.path.join(Y, os.path.basename(image_path))
...
def write_images_to_tfr_short(store_dir, image_paths, filename: str = "train"):
    ...
    for image_path in image_paths:
        current_image, current_label = read_image(image_path)
...
def generate_tfrecords(parameters):
    ...
    train_paths, val_paths = split_train_val(
        paths.list_images(os.path.join(
            parameters['dataset']['data_dir']['train'], 'X')))
